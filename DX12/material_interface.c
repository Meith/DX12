...
void get_checkerboard_tex(UINT64 width, UINT64 height, struct material_info *mat_info)
{
        ...
        const UINT64 cell_pitch = row_pitch >> 3;         // The width of a cell in the checkboard texture.
        const UINT64 cell_height = width >> 3;            // The height of a cell in the checkerboard texture.
